...
static int rocket_probe(struct platform_device *pdev)
{
	...
	if (rdev == NULL) {
		...
		rdev = rocket_device_init(drm_dev, &rocket_drm_driver, variant);
		if (IS_ERR(rdev)) {
...
static const struct rocket_variant rk3568_variant = {
	.dma_bits = 32,
	.pc_data_amount_scale = 1,
};

static const struct rocket_variant rk3588_variant = {
	.dma_bits = 40,
	.pc_data_amount_scale = 2,
};

const struct of_device_id rocket_dt_match[] = {
	{ .compatible = "rockchip,rk3568-rknn-core", .data = &rk3568_variant },
	{ .compatible = "rockchip,rk3588-rknn-core", .data = &rk3588_variant },
	{}
};
